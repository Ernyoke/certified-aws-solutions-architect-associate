...
- Read after write consistency for PUTS of new objects, which means:
    - As soon as we upload a new file, we can retrieve it: PUT 200 => GET 200
    - Except if we did a GET before put to see that the object does exist: GET 404 => PUT 200 => GET 404 - eventual consistency
- Eventual consistency is also present for DELETES and PUTS of existing objects: PUT 200 => PUT 200 => GET 200 (although the response is successful, the returned object might be an older version). In case of DELETE we might be able to retrieve it for a short amount of time
- There is no way to get strong consistency for S3!